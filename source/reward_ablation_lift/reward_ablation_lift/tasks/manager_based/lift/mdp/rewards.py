...
def object_is_grasped(
    ...
    # net_forces_w: (num_envs, num_bodies, 3) — total contact force, no object filter
    force_1 = torch.norm(sensor_1.data.net_forces_w[:, 0, :], dim=-1)
    force_2 = torch.norm(sensor_2.data.net_forces_w[:, 0, :], dim=-1)
...
def object_is_lifted(
    ...
    is_grasped = object_is_grasped(env, normal_force_threshold=3.0)
    return reward * is_grasped.float()
